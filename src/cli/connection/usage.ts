...
COMMANDS:
  connection add <alias> [connection-string] [--credential <name>] [options]
    ...
    --driver pg|cockroachdb|sqlite|mysql|mariadb|duckdb|snowflake  Database driver (auto-detected from URL/extension if omitted).
    --host <host>             Database host (pg, cockroachdb, mysql, mariadb).
    --port <port>             Database port (pg, cockroachdb, mysql, mariadb).
    --database <db>           Database name (pg, mysql, mariadb, snowflake).
...
AD-HOC: -c also accepts file paths (./data.db, ./data.duckdb) and URLs (postgres://..., cockroachdb://..., mysql://..., mariadb://..., duckdb://..., snowflake://...) without prior setup.
