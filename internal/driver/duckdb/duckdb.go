...
func (c *duckdbConn) exec(ctx context.Context, sql string) ([]map[string]any, error) {
	...
	// Forward DuckDB warnings (deprecations, extension loading) to stderr
	if cmd.Stderr != nil {
		// Already captured above via CombinedOutput path — not applicable here
	}

	return parseNDJSON(string(stdout))
}

// execWithStderr captures both stdout and stderr separately so warnings
...
// orderedKeys returns map keys in JSON-parse order (which in Go is not guaranteed,
// so we re-parse the first line to get deterministic ordering).
// For simplicity, we just return sorted keys — DuckDB NDJSON column order
// matches the SELECT order, but Go maps don't preserve insertion order.
...
func classifyError(message string) error {
	...
	if strings.Contains(firstLine, "Catalog Error") {
		hint := "Use 'schema tables' to see available tables."
